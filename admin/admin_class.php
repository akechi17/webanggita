...
class Action
{
	...
	{
		extract($_POST);
		$get = $this->db->query("SELECT * FROM lelangs where product_id = $product_id order by harga_lelang desc limit 1 ");
	...
	function save_bid()
	{
		extract($_POST);
		$data = "";
		$chk = $this->db->query("SELECT * FROM lelangs where product_id = $product_id order by harga_lelang desc limit 1 ");
		...
		if (empty($id)) {
			$save = $this->db->query("INSERT INTO lelangs set " . $data);
		} else {
			$save = $this->db->query("UPDATE lelangs set " . $data . " where id=" . $id);
		}
		if ($save)
			return 1;
	}
